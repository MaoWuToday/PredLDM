...
class STDiTBlock(nn.Module):
    ...
    def forward(self, x, y, t, mask=None, tpe=None, x_mask=None, t0=None):
        ...
        # cross attn
        x = x + self.cross_attn(x, y, mask)  # change to x = x + self.cross_attn(x, x', mask)
        # test for x + x'
        # x = x + self.cross_attn(x, x, mask)  # change to x = x + self.cross_attn(x, x', mask)
        # print('y is conditionnaed !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
...
class STDiT(nn.Module):
    def __init__(
            self,
            input_size=(1, 32, 32),  # [10 8 8]  actually is [10 64 64]
            in_channels=4,  # 4
            patch_size=(1, 2, 2),
            hidden_size=1152,
            depth=28,
        ...
        num_patches = np.prod([input_size[i] // patch_size[i] for i in
                               range(3)])  # actually is 10240, not the vae get lantet size caused result
    ...
    def forward(self, x, timestep, y, mask=None, x_mask=None, **kwargs):
        ...
        x = x.to(dtype)  # [2, 4, 10, 64, 64]
        timestep = timestep.to(dtype)
        y = y.to(dtype)

        # embedding
        x = self.x_embedder(x)  # [B, N, C]             [2, 10240, 1152]  T H W = N, hidden size 1152 ok
        ...
        # embedding
        y = self.x_embedder(y)  # [B, N, C]             [2, 10240, 1152]  T H W = N, hidden size 1152 ok
        y = rearrange(y, "B (T S) C -> B T S C", T=self.num_temporal, S=self.num_spatial)
        y = y + self.pos_embed
        y = rearrange(y, "B T S C -> B (T S) C")
...
def STDiT_XL_2(from_pretrained=None, **kwargs):
    # model = STDiT(depth=8, hidden_size=128, patch_size=(1, 2, 2), num_heads=8, **kwargs)
    model = STDiT(depth=16, hidden_size=512, patch_size=(1, 2, 2), num_heads=8, **kwargs)
    # model = STDiT(depth=28, hidden_size=1152, patch_size=(1, 2, 2), num_heads=16, **kwargs)
    if from_pretrained is not None:
        print("Time: " + datetime.now().strftime(
            '%Y-%m-%d %H:%M:%S') + " -- Initialize diffusions from pretrained model: {} in pretrained_models".format(
            from_pretrained))
        # hidden_size=1152,        depth=28,        num_heads=16,   is the official settings of pretrained diffusions
