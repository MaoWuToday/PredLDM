...
class Predictor(nn.Module):
    def __init__(self, memory_size):
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ConvTranspose2d(in_channels=64, out_channels=64, kernel_size=(3, 3), stride=1, padding=1,
                               output_padding=0),
            nn.ELU(),
            nn.ConvTranspose2d(in_channels=64, out_channels=1, kernel_size=(3, 3), stride=2, padding=1,
                               output_padding=1))
    ...
    def forward(self, short_x, long_x, out_len, phase):
        ...
        h, c, out_pred = [], [], []
        for layer_i in range(self.convlstm_num):
            zero_state = torch.zeros(batch_size, self.convlstm_in_c[layer_i], memory_feature.size()[2],
                                     memory_feature.size()[3]).to(self.device)
            h.append(zero_state)
            c.append(zero_state)
