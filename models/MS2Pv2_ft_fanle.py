...
class MS2Pv2(nn.Module):
    ...
    def forward(self, x, **kwargs):
        """for fine tune, only the path. [input + prompt] -> enc -> mid -> dec -> output"""
        B, T, C, H, W = x.shape
        x = x.view(B * T, C, H, W)
        # x + prompt -> x
        ...
        """1. upper branch, task, video prediction. [input + prompt] -> output"""
        B, T, C, H, W = x.shape
        x = x.view(B * T, C, H, W)
        # x + prompt -> x
        x_upper = self.prompt(x)
        ...
        embed_upper, skip_upper = self.enc(x_upper)
        # siamese decoding
        Y_pred = self.dec(embed_upper, skip_upper)
        Y_pred = Y_pred.reshape(B, T, C, H, W)

        """2. lower branch: auto encoding. [input * mask] -> input"""
        # x * mask -> x
        x_lower = self.mask(x)
        # siamese encoding
        embed_lower, skip_lower = self.enc(x_lower)
        _, C_, H_, W_ = embed_lower.shape
        # temporal translator
        z = embed_lower.view(B, T, C_, H_, W_)
        z = self.translator(z)
        z = z.reshape(B * T, C_, H_, W_)
        # siamese decoding
        Y_in = self.dec(z, skip_lower)
        Y_in = Y_in.reshape(B, T, C, H, W)
