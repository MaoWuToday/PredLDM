...
class HumanDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for j in range(begin, end, self.step):
            ...
            # except:
            #     image = np.zeros([self.input_shape[2], self.input_shape[3], 3])
            #     print('Image {} is none'.format(file_name))
            try:
                image = image[:, :, ::-1]       # rgb to bgr
                # print(image.shape)
            except:
                image = np.zeros([self.input_shape[2], self.input_shape[2], 3])
                print('Image {} is none'.format(file_name))
                # print(self.input_shape[2], self.input_shape[2])
                # print(image.shape)
...
def load_data_human(batch_size, val_batch_size, data_root, num_workers=4,
    ...
    dataloader_train = DataLoader(train_set, batch_size, shuffle=True, num_workers=num_workers)
    ...
    #                                 num_workers=num_workers,
    #                                 distributed=distributed, use_prefetcher=use_prefetcher)
    dataloader_test = DataLoader(test_set, val_batch_size, shuffle=False, num_workers=num_workers)
...
if __name__ == '__main__':
    ...
    # dist_params = dict(launcher='pytorch', backend='gloo', init_method='env://', world_size=1)
    # init_dist(**dist_params)

    dataloader_train, _, dataloader_test = \
        load_data_human(batch_size=16,
                        val_batch_size=4,
                        # data_root='../../data/',
                        data_root=r'F:\PAM-master\data',
                        num_workers=4,
