...
class HKO_7_Dataset(Dataset):
    def __init__(self, root, seq_len, seq_interval=20, transforms=None, train=True, test=False,
                 nonzero_points_threshold=None, aug=None):
        self.root = os.path.join(root, 'hko7')
        self.train = train
        self.test = test
        ...
        if self.train:
            ...
            # f = open(os.path.join(self.root, 'hko7_rainy_train_days.txt'), 'r')
        elif self.test:
            f = open(os.path.join(self.root, 'hko7_rainy_test_days_statistics.txt'), 'r')
            # f = open(os.path.join(self.root, 'hko7_rainy_test_days.txt'), 'r')
        else:
            f = open(os.path.join(self.root, 'hko7_rainy_valid_days_statistics.txt'), 'r')
            # f = open(os.path.join(self.root, 'hko7_rainy_valid_days.txt'), 'r')
        rainy_days = []
    ...
    def __getitem__(self, item):
        ...
        for i in range(self.seq_len):
            # print('Open Image: {} '.format(rainy_day_imgs_paths[index + i]))
            rainy_day_img = Image.open(rainy_day_imgs_paths[index + i])
            # rainy_day_img = rainy_day_img.resize((256, 256), Image.BILINEAR)
            rainy_day_img = rainy_day_img.resize((configs.img_width, configs.img_height), Image.BILINEAR)
...
class HKO_7_samples(Dataset):
    ...
    def __getitem__(self, item):
        ...
        for i in range(len(test_sample_imgs_paths)):
            ...
            test_sample_img = test_sample_img.resize((configs.img_width, configs.img_height), Image.BILINEAR)
